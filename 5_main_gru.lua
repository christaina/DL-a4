...
valLogger = optim.Logger(paths.concat(logdir, 'val.log'))

params = {
                batch_size=20, -- minibatch
                seq_length=30, -- unroll length
                layers=2,
                decay=4,
                rnn_size=250, -- hidden unit size
                dropout=0.5, 
...
fi = io.open(paths.concat(logdir,"params"), "w")

-- sets the default output file as test.lua
for key,value in pairs(params) do fi:write(key,": ",value) fi:write("\n") end
io.close(fi)
...
function print_preds(preds)
	for i =1,preds:size()[1] do
		print(inv_vocab_map[preds[i][1]])
	end
end
...
function run_valid()
   ...
   valLogger:add{['% mean class accuracy (validation set)'] = g_f3(torch.exp(perp/len))}
...
function run_test()
	...
	val,ind = torch.max(preds,2)
	print_preds(ind)
        perp = perp + perp_tmp[1]
        g_replace_table(model.s[0], model.s[1])
    end
   testLogger:add{['% mean class accuracy (test set)'] = g_f3(torch.exp(perp/(len-1)))}
...
inv_vocab_map = ptb.inv_vocab_map
vocab_map=ptb.vocab_map
torch.save('data/vocab_map.tb',vocab_map)
torch.save('data/inv_vocab_map.tb',inv_vocab_map)
print('vocab map saved')
...
while epoch < params.max_max_epoch do
    ...
    -- change to 5 --> change back to 10 later?
    if step % torch.round(epoch_size / 10) == 10 then
        wps = torch.floor(total_cases / torch.toc(start_time))
        since_beginning = g_d(torch.toc(beginning_time) / 60)
   trainLogger:add{['% mean class accuracy (train set)'] = g_f3(torch.exp(perps:mean()))}
	...
	torch.save(paths.concat(logdir,'model.net'),model)
	print("model saved.")
    end
end
run_test()
